...
func (h *CompletionHandler) listModels(ctx *fasthttp.RequestCtx) {
	...
	bifrostListModelsReq := preparePublicListModelsRequest(ctx, provider)
...
func preparePublicListModelsRequest(ctx *fasthttp.RequestCtx, provider string) *schemas.BifrostListModelsRequest {
	pageSize := 0
	...
	for key, value := range ctx.QueryArgs().All() {
		name := string(key)
		switch name {
		case "provider", "page_size", "page_token", "unfiltered":
			continue
		default:
			extraParams[name] = string(value)
		}
	}
...
func (h *CompletionHandler) listModelsCached(ctx *schemas.BifrostContext, req *schemas.BifrostListModelsRequest) (*schemas.BifrostListModelsResponse, *schemas.BifrostError) {
	if !listModelsCacheable(ctx, req) {
		return h.fetchListModels(ctx, req)
	}
...
func listModelsCacheable(ctx *schemas.BifrostContext, req *schemas.BifrostListModelsRequest) bool {
	if req == nil || len(req.ExtraParams) > 0 {
		return false
	}
	if bifrost.GetStringFromContext(ctx, schemas.BifrostContextKeyAPIKeyName) != "" ||
		bifrost.GetStringFromContext(ctx, schemas.BifrostContextKeyAPIKeyID) != "" {
		return false
	}
	_, hasDirectKey := ctx.Value(schemas.BifrostContextKeyDirectKey).(schemas.Key)
	return !hasDirectKey
}

func (h *CompletionHandler) fetchListModels(ctx *schemas.BifrostContext, req *schemas.BifrostListModelsRequest) (*schemas.BifrostListModelsResponse, *schemas.BifrostError) {
	if h.client == nil {
		return nil, &schemas.BifrostError{
			IsBifrostError: false,
			Error:          &schemas.ErrorField{Message: "failed to list models"},
	...
	if req.Provider == "" {
		return h.client.ListAllModels(ctx, req)
	}
	return h.client.ListModelsRequest(ctx, req)
}
