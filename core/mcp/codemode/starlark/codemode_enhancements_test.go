...
func TestReadToolFileMarksLookupFailures(t *testing.T) {
	mode := NewStarlarkCodeMode(&codemcp.CodeModeConfig{
		BindingLevel:         schemas.CodeModeBindingLevelServer,
		ToolExecutionTimeout: time.Second,
	}, nil)
	...
	for _, args := range []string{
		`{"fileName":"servers/ghost.pyi"}`,
		`{"fileNames":["servers/alpha.pyi","servers/ghost.pyi"]}`,
	} {
		msg, err := mode.handleReadToolFile(context.Background(), toolCallArgs("readToolFile", args))
		if err != nil {
			t.Fatalf("handleReadToolFile(%s) error: %v", args, err)
		}
		if msg.ChatToolMessage == nil || msg.ChatToolMessage.IsError == nil || !*msg.ChatToolMessage.IsError {
			t.Fatalf("lookup failure must set IsError for %s, got %#v", args, msg.ChatToolMessage)
		}
	}
}
...
func TestExecuteToolCodeOmitEnvironmentFooter(t *testing.T) {
	...
	omit := NewStarlarkCodeMode(&codemcp.CodeModeConfig{
		BindingLevel:          schemas.CodeModeBindingLevelServer,
		ToolExecutionTimeout:  time.Second,
		OmitEnvironmentFooter: schemas.Ptr(true),
	}, nil)
	...
	omit.UpdateConfig(&codemcp.CodeModeConfig{BindingLevel: schemas.CodeModeBindingLevelTool})
	if !omit.omitEnvironmentFooter() {
		t.Fatal("unrelated code-mode config update reset the footer setting")
	}
	omit.UpdateConfig(&codemcp.CodeModeConfig{OmitEnvironmentFooter: schemas.Ptr(false)})
	if omit.omitEnvironmentFooter() {
		t.Fatal("explicit false update did not restore the environment footer")
	}
}
